		...
		<>
			...
			<div className="menu-item">
				...
				<select
					...
					name="section-margin"
					className="bg-transparent text-black/80 outline-none border-0 cursor-pointer animation-all max-w-[70px] truncate"
				>
					...
					{SECTIONS.map((item, index) => (
						<option
							key={index}
							value={item.value}
							className="bg-zinc-900 text-black max-w-[70px] truncate"
						>
							{item.label}
						</option>
					))}
			...
			<div className="menu-item">
				...
				<div className="flex items-center justify-end w-full gap-0.5">
					<input
						...
						className="outline-0 border-0 bg-transparent border-b border-transparent border-white/50 text-xs text-black/80 w-1/3 appearance-none relative"
						value={customMargin}
						onChange={(e) => setCustomMargin(e.target.value)}
					/>
					<span className="text-xs text-black/40">px</span>
				</div>
			</div>
