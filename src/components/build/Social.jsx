...
import { FaLinkedin, FaGithub, FaTwitter, FaFacebook, FaInstagram, FaGlobe, FaDribbbleSquare } from 'react-icons/fa';
		...
		<div className="my-14 lg:my-20 px-10 flex flex-col gap-2">
				...
				{SOCIALS.map((social) => (
					<div key={social.name.toLowerCase()} className="mb-4">
						...
						<input
							...
							onChange={(e) => setStore(`socialLinks.${social.name.toLowerCase()}`, e.target.value)}
			...
			<div className="flex justify-between mt-6">
				<button 
					...
					onClick={() => document.querySelector('button[data-section="personal"]').click()}
				>
					Previous
				</button>
				<button 
					className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700"
					onClick={() => document.querySelector('button[data-section="summary"]').click()}
				>
					Next
				</button>
			</div>
